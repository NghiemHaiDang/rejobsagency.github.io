...
class Workio_Widget_Candidate_Filter_Horizontal extends Apus_Widget {
	
	function __construct() {
		parent::__construct(
			'candidate_filter_horizontal_widget',
			esc_html__( 'Candidate Filter (Horizontal)', 'workio' ),
			array(
				'description' => esc_html__( 'Filter for filtering candidates.', 'workio' ),
			)
		);
	...
	public function getTemplate() {
        $this->template = 'candidate-filter-horizontal.php';
    }
	...
	public function form( $instance ) {
		...
		?>
		<div id="filter-candidate-<?php echo esc_attr($_id); ?>" class="filter-candidate-horizontal">
			<!-- TITLE -->
			<p>
			...
			<ul class="wp-job-board-filter-fields workio-filter-candidate-fields">
				<?php
				$fields = workio_candidate_get_filter_fields();
				if ( ! empty( $sort ) ) {
			...
			<hr>
			<div class="workio-filter-candidate-adv-fields-wrapper">
				<h3><?php echo esc_html__('Advance Filter Fields', 'workio'); ?></h3>
				<ul class="wp-job-board-filter-fields workio-filter-candidate-adv-fields">
					<?php
					$fields = workio_candidate_get_filter_fields();
					if ( ! empty( $sort_adv ) ) {
...
if ( function_exists('apus_framework_reg_widget') ) {
    apus_framework_reg_widget('Workio_Widget_Candidate_Filter_Horizontal');
}