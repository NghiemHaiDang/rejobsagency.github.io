<?php
/**
 * workio functions and definitions
 *
...
define( 'WORKIO_THEME_VERSION', '1.0.8' );
define( 'WORKIO_DEMO_MODE', false );

if ( ! isset( $content_width ) ) {
	$content_width = 660;
}

if ( ! function_exists( 'workio_setup' ) ) :
/**
 ...
 * @since Workio 1.0
 */
function workio_setup() {

	/*
	 * Make theme available for translation.
	 * Translations can be filed in the /languages/ directory.
	 * If you're building a theme based on workio, use a find and replace
	 * to change 'workio' to the name of your theme in all the template files
	 */
	load_theme_textdomain( 'workio', get_template_directory() . '/languages' );
	...
	add_image_size( 'workio-logo-size', 180, 9999, false );

	// This theme uses wp_nav_menu() in two locations.
	register_nav_menus( array(
		'primary' => esc_html__( 'Primary Menu', 'workio' ),
		'employer-menu' => esc_html__( 'Employer Menu', 'workio' ),
		'candidate-menu' => esc_html__( 'Candidate Menu', 'workio' ),
		'employee-menu' => esc_html__( 'Employee Menu', 'workio' ),
	) );
	...
	add_theme_support( 'custom-background', apply_filters( 'workio_custom_background_args', array(
	...
	add_editor_style( array( 'css/style-editor.css', workio_get_fonts_url() ) );

	workio_get_load_plugins();
}
endif; // workio_setup
add_action( 'after_setup_theme', 'workio_setup' );

/**
 * Load Google Front
 */

function workio_get_fonts_url() {
    $fonts_url = '';
    ...
    */
    $poppins = _x( 'on', 'Poppins font: on or off', 'workio' );
    $muli = _x( 'on', 'Muli Sans font: on or off', 'workio' );

    if ( 'off' !== $poppins || 'off' !== $muli ) {
        $font_families = array();
 ...
 * @since Workio 1.0
 */
function workio_enqueue_styles() {
	
	// load font
	wp_enqueue_style( 'workio-theme-fonts', workio_get_fonts_url(), array(), null );
	...
	// main style
	wp_enqueue_style( 'workio-template', get_template_directory_uri() . '/css/template.css', array(), '1.0' );
	
	$custom_style = workio_custom_styles();
	if ( !empty($custom_style) ) {
		wp_add_inline_style( 'workio-template', $custom_style );
	}
	wp_enqueue_style( 'workio-style', get_template_directory_uri() . '/style.css', array(), '1.0' );

	wp_deregister_style('yith-wcwl-font-awesome');
}
add_action( 'wp_enqueue_scripts', 'workio_enqueue_styles', 100 );

function workio_login_enqueue_styles() {
	wp_enqueue_style( 'font-awesome', get_template_directory_uri() . '/css/font-awesome.css', array(), '4.5.0' );
	wp_enqueue_style( 'workio-login-style', get_template_directory_uri() . '/css/login-style.css', array(), '1.0' );
}
add_action( 'login_enqueue_scripts', 'workio_login_enqueue_styles', 10 );
/**
 * Enqueue scripts.
 *
 * @since Workio 1.0
 */
function workio_enqueue_scripts() {
	...
	wp_localize_script( 'countdown', 'workio_countdown_opts', array(
		'days' => esc_html__('Days', 'workio'),
		'hours' => esc_html__('Hrs', 'workio'),
		'mins' => esc_html__('Mins', 'workio'),
		'secs' => esc_html__('Secs', 'workio'),
	));
	...
	// main script
	wp_register_script( 'workio-functions', get_template_directory_uri() . '/js/functions.js', array( 'jquery' ), '20150330', true );
	wp_localize_script( 'workio-functions', 'workio_ajax', array(
		'ajaxurl' => esc_url(admin_url( 'admin-ajax.php' )),
		'previous' => esc_html__('Previous', 'workio'),
		'next' => esc_html__('Next', 'workio'),
		'mmenu_title' => esc_html__('Menu', 'workio')
	));
	wp_enqueue_script( 'workio-functions' );
	
	wp_add_inline_script( 'workio-functions', "(function(html){html.className = html.className.replace(/\bno-js\b/,'js')})(document.documentElement);" );
}
add_action( 'wp_enqueue_scripts', 'workio_enqueue_scripts', 1 );
 ...
 */
function workio_search_form_modify( $html ) {
	return str_replace( 'class="search-submit"', 'class="search-submit screen-reader-text"', $html );
}
add_filter( 'get_search_form', 'workio_search_form_modify' );

/**
 * Function get opt_name
 *
 */
function workio_get_opt_name() {
	return 'workio_theme_options';
}
add_filter( 'apus_framework_get_opt_name', 'workio_get_opt_name' );


function workio_register_demo_mode() {
	if ( defined('WORKIO_DEMO_MODE') && WORKIO_DEMO_MODE ) {
		return true;
	}
	return false;
}
add_filter( 'apus_framework_register_demo_mode', 'workio_register_demo_mode' );

function workio_get_demo_preset() {
	$preset = '';
    if ( defined('WORKIO_DEMO_MODE') && WORKIO_DEMO_MODE ) {
...
function workio_get_config($name, $default = '') {
	global $workio_options;
    if ( isset($workio_options[$name]) ) {
        return $workio_options[$name];
    }
    return $default;
}

function workio_set_exporter_ocdi_settings_option_keys($option_keys) {
	return array(
...
add_filter( 'apus_exporter_ocdi_settings_option_keys', 'workio_set_exporter_ocdi_settings_option_keys' );

function workio_disable_one_click_import() {
	return false;
}
add_filter('apus_frammework_enable_one_click_import', 'workio_disable_one_click_import');

function workio_widgets_init() {
	register_sidebar( array(
		'name'          => esc_html__( 'Sidebar Default', 'workio' ),
		'id'            => 'sidebar-default',
		'description'   => esc_html__( 'Add widgets here to appear in your Sidebar.', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Jobs filter sidebar', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Jobs filter top sidebar', 'workio' ),
		'id'            => 'jobs-filter-top-sidebar',
		'description'   => esc_html__( 'Add widgets here to appear in your sidebar.', 'workio' ),
		'before_widget' => '<aside class="no-margin widget %2$s">',
		'after_widget'  => '</aside>',
		'before_title'  => '<h2 class="widget-title"><span>',
		'after_title'   => '</span></h2>',
	) );
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Job single sidebar', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Employers filter sidebar', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Employers filter top sidebar', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Employer single sidebar', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Candidates filter sidebar', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Candidates filter top sidebar', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Candidate single sidebar', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'User Profile sidebar', 'workio' ),
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Blog sidebar', 'workio' ),
		'id'            => 'blog-sidebar',
		'description'   => esc_html__( 'Add widgets here to appear in your sidebar.', 'workio' ),
		'before_widget' => '<aside class="widget %2$s">',
		'after_widget'  => '</aside>',
	...
	register_sidebar( array(
		'name'          => esc_html__( 'Shop sidebar', 'workio' ),
		'id'            => 'shop-sidebar',
		'description'   => esc_html__( 'Add widgets here to appear in your sidebar.', 'workio' ),
		'before_widget' => '<aside class="widget %2$s">',
		'after_widget'  => '</aside>',
...
add_action( 'widgets_init', 'workio_widgets_init' );

function workio_get_load_plugins() {

	$plugins[] = array(
		'name'                     => esc_html__( 'Apus Framework For Themes', 'workio' ),
        'slug'                     => 'apus-framework',
        'required'                 => true ,
	...
	$plugins[] = array(
		'name'                     => esc_html__( 'Elementor Page Builder', 'workio' ),
	    'slug'                     => 'elementor',
	    'required'                 => true,
	);

	$plugins[] = array(
		'name'                     => esc_html__( 'Cmb2', 'workio' ),
	    'slug'                     => 'cmb2',
	    'required'                 => true,
	);

	$plugins[] = array(
		'name'                     => esc_html__( 'MailChimp for WordPress', 'workio' ),
	    'slug'                     => 'mailchimp-for-wp',
	    'required'                 =>  true
	);

	$plugins[] = array(
		'name'                     => esc_html__( 'Contact Form 7', 'workio' ),
	    'slug'                     => 'contact-form-7',
	    'required'                 => true,
	);
	...
	// woocommerce plugins
	$plugins[] = array(
		'name'                     => esc_html__( 'Woocommerce', 'workio' ),
	    'slug'                     => 'woocommerce',
	    'required'                 => true,
	);
	...
	// Job plugins
	$plugins[] = array(
		'name'                     => esc_html__( 'WP Job Board', 'workio' ),
        'slug'                     => 'wp-job-board',
        'required'                 => true ,
	...
	$plugins[] = array(
		'name'                     => esc_html__( 'WP Job Board - WooCommerce Paid Listings', 'workio' ),
	...
	$plugins[] = array(
		'name'                     => esc_html__( 'WP Private Message', 'workio' ),
	...
	$plugins[] = array(
        'name'                  => esc_html__( 'One Click Demo Import', 'workio' ),
...
if ( defined( 'APUS_FRAMEWORK_REDUX_ACTIVED' ) ) {
	...
	define( 'WORKIO_REDUX_FRAMEWORK_ACTIVED', true );
}
if( workio_is_cmb2_activated() ) {
	require get_template_directory() . '/inc/vendors/cmb2/page.php';
	define( 'WORKIO_CMB2_ACTIVED', true );
}
if( workio_is_woocommerce_activated() ) {
	...
	define( 'WORKIO_WOOCOMMERCE_ACTIVED', true );
}
...
if ( workio_is_wp_job_board_wc_paid_listings_activated() ) {
	require get_template_directory() . '/inc/vendors/wp-job-board-wc-paid-listings/functions.php';
}

if( workio_is_apus_framework_activated() ) {
	...
	if ( workio_is_wp_job_board_activated() ) {
		require get_template_directory() . '/inc/widgets/candidate-contact-info.php';
		...
		require get_template_directory() . '/inc/widgets/employer-contact-info.php';
		...
		require get_template_directory() . '/inc/widgets/employer-filter-horizontal.php';
		require get_template_directory() . '/inc/widgets/candidate-filter-horizontal.php';
		
		require get_template_directory() . '/inc/widgets/job-list.php';
	}
	define( 'WORKIO_FRAMEWORK_ACTIVED', true );
}
if ( workio_is_wp_private_message() ) {
...
function workio_register_post_types($post_types) {
...
add_filter( 'apus_framework_register_post_types', 'workio_register_post_types' );
...
if( function_exists('acf_add_options_page') ) {
	acf_add_options_page(array(
		'page_title' 	=> 'Banner options', // Title hiển thị khi truy cập vào Options page
		'menu_title'	=> 'Banner options', // Tên menu hiển thị ở khu vực admin
		'menu_slug' 	=> 'banner-settings', // Url hiển thị trên đường dẫn của options page
...
function search_bar() {
    if (isset($_GET['s'])) $keyx = $_GET['s']; else  $keyx = 'Tìm công việc/Nhà tuyển dụng...';
    echo '<div class="form-group">
	<form action="/" method="GET">
	<div class="input-group form-group">
		<input class="form-control" name="s" placeholder="'.$keyx.'" type="text" required="">
		<span class="input-group-btn">
		<button type="submit" class="btn btn-secondary" id="getlink" value=""><span id="text_link">Tìm kiếm <i class="fa fa-search form-control-feedback"></i></span></button>
		</span>
	</div>
</form>
</div>';
}
add_shortcode( 'khung_tim_kiem', 'search_bar' );