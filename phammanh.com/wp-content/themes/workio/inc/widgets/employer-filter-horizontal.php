...
class Workio_Widget_Employer_Filter_Horizontal extends Apus_Widget {
	
	function __construct() {
		parent::__construct(
			'employer_filter_horizontal_widget',
			esc_html__( 'Employer Filter (Horizontal)', 'workio' ),
			array(
				'description' => esc_html__( 'Filter for filtering employers.', 'workio' ),
			)
		);
	...
	public function getTemplate() {
        $this->template = 'employer-filter-horizontal.php';
    }
	...
	public function form( $instance ) {
		...
		?>
		<div id="filter-employer-<?php echo esc_attr($_id); ?>" class="filter-employer-horizontal">
			<!-- TITLE -->
			<p>
			...
			<ul class="wp-job-board-filter-fields workio-filter-employer-fields">
				<?php
				$fields = workio_employer_get_filter_fields();
				if ( ! empty( $sort ) ) {
			...
			<hr>
			<div class="workio-filter-employer-adv-fields-wrapper">
				<h3><?php echo esc_html__('Advance Filter Fields', 'workio'); ?></h3>
				<ul class="wp-job-board-filter-fields workio-filter-employer-adv-fields">
					<?php
					$fields = workio_employer_get_filter_fields();
					if ( ! empty( $sort_adv ) ) {
...
if ( function_exists('apus_framework_reg_widget') ) {
    apus_framework_reg_widget('Workio_Widget_Employer_Filter_Horizontal');
}