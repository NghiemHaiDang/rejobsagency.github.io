...
function workio_ocdi_import_files() {
    $demos = array();
    for ($i=1; $i <= 7; $i++) {
        $demos[] = array(
            ...
            'local_import_redux'           => array(
                array(
                    ...
                    'option_name' => 'workio_theme_options',
                ),
            ),
            'import_preview_image_url'     => trailingslashit( get_template_directory_uri() ) . 'inc/vendors/one-click-demo-import/default/h-'.$i.'.jpg',
            'import_notice'                => esc_html__( 'Import process may take 5-10 minutes. If you facing any issues please contact our support.', 'workio' ),
            'preview_url'                  => 'https://apusthemes.com/wp-demo/workio/',
        );
    }
    return apply_filters( 'workio_ocdi_files_args', $demos );
}
add_filter( 'pt-ocdi/import_files', 'workio_ocdi_import_files' );

function workio_ocdi_after_import_setup( $selected_import ) {
    // Assign menus to their locations.
    $main_menu       = get_term_by( 'name', 'Main Menu', 'nav_menu' );
    $employer_menu      = get_term_by( 'name', 'Employer Menu', 'nav_menu' );
    $candidate_menu   = get_term_by( 'name', 'Candidate Menu', 'nav_menu' );
    ...
    switch ($selected_import['import_file_name']) {
        ...
        case 'Home 6':
            ...
            break;
        case 'Home 7':
            $front_page_id = get_page_by_title( 'Freelancing' );
            update_option( 'page_on_front', $front_page_id->ID );
    ...
    if ( file_exists($file) ) {
        workio_ocdi_import_settings($file);
    }
}
add_action( 'pt-ocdi/after_import', 'workio_ocdi_after_import_setup' );

function workio_ocdi_import_settings($file) {
    ...
    if ( !empty($datas['page_options']) ) {
        workio_ocdi_import_page_options($datas['page_options']);
    }
    if ( !empty($datas['metadata']) ) {
        workio_ocdi_import_some_metadatas($datas['metadata']);
    }
}

function workio_ocdi_import_page_options($datas) {
    if ( $datas ) {
...
function workio_ocdi_import_some_metadatas($datas) {
    if ( $datas ) {
...
function workio_ocdi_before_widgets_import() {
...
add_action( 'pt-ocdi/before_widgets_import', 'workio_ocdi_before_widgets_import' );