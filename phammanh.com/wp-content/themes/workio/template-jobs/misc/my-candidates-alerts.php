...
<div class="box-dashboard-wrapper">
					...
					<tbody>
						...
						foreach ($alerts->posts as $alert_id) {
							...
							<tr <?php post_class('job-alert-wrapper'); ?>>
							    ...
							    <td>
									<a href="javascript:void(0)" class="btn-remove-job-alert text-danger btn-loading" data-alert_id="<?php echo esc_attr($alert_id); ?>" data-nonce="<?php echo esc_attr(wp_create_nonce( 'wp-job-board-remove-candidate-alert-nonce' )); ?>"><i class="ti-trash"></i><?php esc_html_e('Remove', 'workio'); ?></a>
								</td>
							</tr><!-- #post-## -->
	...
	<?php } else { ?>
		<div class="not-found"><?php esc_html_e('No candidate alert found.', 'workio'); ?></div>
	<?php } ?>
	</div>
</div>