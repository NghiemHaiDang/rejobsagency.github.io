		...
		if ( empty($application_deadline_date) || strtotime($application_deadline_date) >= strtotime('now') ) {
			...
			if ( $apply_type == 'external' ) {
				...
				if ( !is_user_logged_in() || !WP_Job_Board_User::is_candidate() ) {
					?>
					<a href="javascript:void(0);" class="btn btn-apply btn-apply-job-internal-required"><?php //esc_html_e('Apply Now', 'wp-job-board'); ?>Ứng tuyển<i class="next flaticon-right-arrow"></i></a>
					<?php
